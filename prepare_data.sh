...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "选项:"
            echo "  --input FILE              原始数据文件路径 (默认: dataset/benchmark_python_v3.0.jsonl)"
            echo "  --output_jsonl FILE       输出 JSONL 文件路径 (默认: benchmark_python_3.0_repo2run.jsonl)"
            echo "  --output_commands FILE    输出命令文件路径 (默认: commands.txt)"
...
echo ""
echo "接下来你可以:"
echo "  1. 执行 ./run.sh 来构建环境"
echo "  2. 执行 ./run_evaluation.sh 来评估结果"
echo "  3. 或者执行 ./run_all.sh 一键完成所有步骤"
echo ""
echo "================================================================================"
